...
class Boy:
    ...
    def update(self):
        ...
        self.x = clamp(
            25.0,
            self.x,
            server.background.w - 25.0
        )
        self.y = clamp(
            30.0,
            self.y,
            server.background.h - 30.0)

    def handle_event(self, event):
        self.state_machine.handle_event(('INPUT', event))

    def draw(self):
        # x, y 는 배경 월드 좌표계를, sx, sy 즉 화면 좌표로 변환
    ...
    def get_bb(self):
        return (self.x - 20 - server.background.window_left,
                self.y - 50 - server.background.window_bottom,
                self.x + 20 - server.background.window_left,
                self.y + 50 - server.background.window_bottom)
